...
from datasets.datasets import get_australian_dataset
import pdb
...
class Australian(Dataset):
	def __init__(self, cuda, batch_size, testing_mode=False):
		self.ds, self.data_shape = get_australian_dataset(batch_size)
		self.cuda = cuda
...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
	...
	ds_train = Australian(cuda=cuda, batch_size=batch_size, testing_mode=testing_mode)  # training dataset

	latent_dim = 5
	cluster_number = 2
	data_shape = ds_train.data_shape
	# TODO AutoEncoder
	autoencoder = StackedDenoisingAutoEncoder([data_shape, 30, latent_dim], final_activation=None)
	...
	predicted, actual = predict(
		ds_train, model, 690, silent=True, return_actual=True, cuda=cuda
	)
