...
class CachedFashionMNIST(Dataset):
	def __init__(self, train, cuda, testing_mode=False):
		...
		self.ds = FashionMNIST("./datasets/FashionMNIST", download=True, train=train, transform=img_transform)
		self.cuda = cuda
...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
	...
	ds_train = CachedFashionMNIST(train=True, cuda=cuda, testing_mode=testing_mode)  # training dataset

	latent_dim = 10
	cluster_number = 10
	autoencoder = StackedDenoisingAutoEncoder([28 * 28, 500, 500, 2000, latent_dim], final_activation=None)
