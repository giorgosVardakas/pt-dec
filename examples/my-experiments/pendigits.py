...
from datasets.datasets import get_Pendigits_dataset
from evaluation import Evaluator
import pdb

class Pendigits(Dataset):
	def __init__(self, cuda, batch_size, testing_mode=False):
		self.ds, self.data_shape = get_Pendigits_dataset(batch_size)
		self.cuda = cuda
...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
	...
	ds_train = Pendigits(cuda=cuda, batch_size=batch_size, testing_mode=testing_mode)  # training dataset
	...
	predicted, actual = predict(
		ds_train, model, 7000, silent=True, return_actual=True, cuda=cuda
	)
