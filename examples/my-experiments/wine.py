...
from datasets.datasets import get_wine_dataset
import pdb
...
class Wine(Dataset):
	def __init__(self, cuda, batch_size, testing_mode=False):
		self.ds, self.data_shape = get_wine_dataset(batch_size)
		self.cuda = cuda
...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
	...
	ds_train = Wine(cuda=cuda, batch_size=batch_size, testing_mode=testing_mode)  # training dataset
	...
	predicted, actual = predict(
		ds_train, model, 178, silent=True, return_actual=True, cuda=cuda
	)
