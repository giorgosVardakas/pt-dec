...
from datasets.datasets import get_iris_dataset
...
class Iris(Dataset):
	def __init__(self, cuda, batch_size, testing_mode=False):
		self.ds, self.data_shape = get_iris_dataset(batch_size)
		self.cuda = cuda
...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
	...
	ds_train = Iris(cuda=cuda, batch_size=batch_size, testing_mode=testing_mode)  # training dataset
	...
	predicted, actual = predict(
		ds_train, model, 150, silent=True, return_actual=True, cuda=cuda
	)
